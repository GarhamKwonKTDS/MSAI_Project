...
echo "  Admin Backend App Name: $ADMIN_BACKEND_APP_NAME"

# Check if source directory exists
if [ ! -d "admin_backend" ]; then
    echo "❌ Source directory 'admin_backend' not found!"
    exit 1
fi
...
if az webapp show --name $ADMIN_BACKEND_APP_NAME --resource-group $RESOURCE_GROUP &>/dev/null; then
...
# Create new web app
echo ""
echo "🌐 Creating Admin Backend Web App..."
az webapp create \
    --resource-group $RESOURCE_GROUP \
...
TEMP_DIR="temp_admin_backend_$(date +%s)"
mkdir -p $TEMP_DIR

# Copy source code
cp -r admin_backend/* $TEMP_DIR/
...
cd $TEMP_DIR
zip -r ../admin_backend_deploy.zip .
cd ..
...
az webapp deployment source config-zip \
    --resource-group $RESOURCE_GROUP \
    --name $ADMIN_BACKEND_APP_NAME \
    --src admin_backend_deploy.zip

# Cleanup
rm -rf $TEMP_DIR
rm admin_backend_deploy.zip

ADMIN_BACKEND_URL="https://${ADMIN_BACKEND_APP_NAME}.azurewebsites.net"

echo ""
echo "✅ Admin Backend deployed successfully!"
echo "================================"
echo "App Name: $ADMIN_BACKEND_APP_NAME"
echo "URL: $ADMIN_BACKEND_URL"
echo "Health Check: ${ADMIN_BACKEND_URL}/health"
echo ""
echo "🔍 To check logs:"
echo "   az webapp log tail --name $ADMIN_BACKEND_APP_NAME --resource-group $RESOURCE_GROUP"
echo ""
echo "🔄 To restart:"
echo "   az webapp restart --name $ADMIN_BACKEND_APP_NAME --resource-group $RESOURCE_GROUP"